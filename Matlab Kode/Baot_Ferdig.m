...
angle_offset = [th1_prev th2_prev+pi/2 th3_prev th4_prev];
theta_initial = [th1_prev, th2_prev+pi/2, th3_prev th4_prev]
...
hold off;
angle_start = [0 0 0 0]
P = fkine4DOF(DH, [Th_1 Th_2+pi/2 Th_3 Th_4], angle_start)
P = eval(P)
...
point1.positions =  [Th_1, -Th_2, -Th_3, -Th_4, 0]
point1.velocities = [0.01, 0.01, 0.01, 0.01, 0.01];
point1.accelerations = [0.01, 0.01, 0.01, 0.01, 0.01];
point1.effort = [0.01, 0.01, 0.01, 0.01, 0.01];
point1.time_from_start.sec = int32(4);

% Sending message to ROS
trajMsg.points = [point1];
...
J = Robot.ikine(T,[0 pi/2 0 0 0],'mask',[1 1 1 0 0 0])*180/pi

set(handles.theta_1, 'string', J(1));
set(handles.theta_2, 'string', J(2)-90);
